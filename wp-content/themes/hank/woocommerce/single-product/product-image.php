	...
	<figure class="woocommerce-product-gallery__wrapper">
		...
		if ( has_post_thumbnail() ) {
			$image = hank_get_image_resized( array(
				...
				'crop'     => hank_option( 'product__imageSizeCrop' ),
				'atts'     => $attributes
			) );

			$thumbnail       = hank_get_image_resized( array(
				'image_id' => $post_thumbnail_id,
				'size'     => hank_option( 'product__thumbnailSize' ),
				'crop'     => hank_option( 'product__thumbnailSizeCrop' )
			) );
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_html__( 'Awaiting product image', 'hank' ) );
			$html .= '</div>';
		}
