...
add_filter( 'acf/prepare_field/name=sidebar', function ( $field ) {
	...
	$field['choices'] = array();
	$field['choices']['default'] = esc_html__( 'Use Theme Default', 'hank' );
...
function hank_override_sidebar_position( $position ) {
	$object = get_post();
...
add_filter( 'hank_sidebar_position', 'hank_override_sidebar_position', 99 );


function hank_override_sidebar_id( $id ) {
...
add_filter( 'hank_sidebar_id', 'hank_override_sidebar_id', 99 );
