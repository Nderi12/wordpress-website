 ...
 * @return  mixed
 */
function hank_option( $id ) {
	static $theme_settings;

	if ( empty( $theme_settings ) ) {
		$theme_settings = apply_filters( 'hank_customize_settings', array() );
	}
 ...
 * @return  strings
 */
function hank_current_post_type() {
 ...
 * @since   1.0.0
 */
function hank_logo( $prefix = '', $attrs = array() ) {
	$logo_key = 'logo';
	...
	$logo = (array) hank_option( $logo_key );
	$logo_retina = (array) hank_option( $logo_retina_key );
	...
	printf( '<img %s />', hank_attributes( $attributes ) );
}


function hank_social_icons( $args ) {
	...
	$positions = hank_option( 'global__social__positions' );

	if ( in_array( $args['location'], $positions ) ) {
		$icons = hank_option( 'global__social__icons' );
		$output = array();
 ...
 * @since   1.0.0
 */
function hank_attributes( $attrs ) {
	$attributes = array();
 ...
 * @since   1.0.0
 */
function hank_google_fonts() {
	// Try to retrive the font list from the cache
	$cached = get_transient( 'hank-fonts' );
	...
	if ( ! $cached ) {
		...
		if ( is_array( $remote_content ) && isset( $remote_content['body'] ) ) {
			...
			set_transient( 'hank-fonts', $cached, 86400 );
		}
	}
 ...
 * @since   1.0.0
 */
function hank_initialize_filesystem_api( $url = '', $action = -1, $name = '_wpnonce' ) {
 ...
 * @return  array
 */
function hank_get_image_resized( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = hank_image_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], $params['crop'] );
			...
			if ( ! empty( $attachment ) ) {
				...
				if ( $p_img ) {
					...
					$thumbnail = '<img ' . hank_attributes_stringify( $attributes ) . ' />';
				}
			}
		}
	} else {
	...
	return apply_filters( 'hank_get_image_resized', array(
		'thumbnail' => $thumbnail,
		'thumbnail_raw' => $p_img,
		'large' => $p_img_large,
	), $attach_id, $params );
 ...
 * @return array
 */
function hank_image_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
 ...
 * @return string
 */
function hank_attributes_stringify( $attributes ) {
	$atts = array();
