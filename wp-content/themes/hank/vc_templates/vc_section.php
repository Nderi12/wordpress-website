...
if ( ! empty( $bg_position ) ) {
	$background_styles[] = sprintf( 'background-position: %s !important;', $bg_position !== 'custom' ? $bg_position : $custom_position);
}
if ( ! empty( $bg_size ) ) {
	$background_styles[] = sprintf( 'background-size: %s !important;', $bg_size !== 'custom' ? $bg_size : $custom_size);
}

$inline_background_style = sprintf( 'style="%s"', join( ' ', $background_styles ) );
$output .= '<section ' . $inline_background_style . ' ' . implode( ' ', $wrapper_attributes ) . '>';
