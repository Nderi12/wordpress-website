...
class Hank_Sample_Data_Worker
{
	...
	public function download() {
		...
		if ( isset($_POST['file']) && hank_initialize_filesystem_api() ) {
			$source           = sprintf( 'https://demo.21lab.co/samples/%s/%s', HANK_ID, $_POST['file'] );
		...
		if ( isset( $_POST['file'] ) && hank_initialize_filesystem_api() ) {
			...
			if ( is_file( $file ) && unzip_file( $file, $this->path() ) ) {
				...
				if ( ! is_file( $data_file ) ) {
					throw new Exception( sprintf( esc_html__( 'Sample data file not found: %s', 'hank' ), $data_file ) );
				}
	...
	public function thumbnails() {
		...
		if ( ! hank_initialize_filesystem_api() ) {
			$this->response( array(
				'code'    => 403,
				'message' => esc_html__( 'Invalid files & folders permissions', 'hank' )
			) );
		}
		...
		if ( ! is_dir( $this->path( 'uploads' ) ) ) {
			$this->response( array(
				'code'    => 500,
				'message' => esc_html__( 'Cannot extract the sample data media files', 'hank' ),
		...
		$this->response( array(
			'code'    => 200,
			'message' => esc_html__( 'Thumbnails generation successfully!', 'hank' )
		) );
	}
	...
	public function finish() {
		...
		if ( isset( $_POST['package'] ) && hank_initialize_filesystem_api() ) {
